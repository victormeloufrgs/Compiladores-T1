...
TAC* run_intermediate_code_generation(AST* tree) {
...
TAC* makeFunctionCallArgs(AST* node, TAC* code0, TAC* code1) {
    ...
    return code1 ? tacJoin(code0, tacJoin(arg, code1)) : tacJoin(code0, arg);
}
...
TAC* makeFunctionDecl(AST* node, TAC* code0, TAC* code1, TAC* code2) {
    ...
    TAC* tacEnd = tacCreate(TAC_FEND, node->symbol, 0, 0);
...
TAC* makeArraySymbDecl(AST* node, TAC* code0, TAC* code1) {
    ...
    TAC* set_values = 0;
    int i = 0;
    decl = tacCreate(TAC_ARR, node->symbol, generateCode(node->son[0]->son[1])->res, 0);

    //percorrer lista de valores e p/ cada uma, add no índice certo a instrução
    /*
    son[0] = type_and_value
    son[0]->son[2] = vet_maybe_value
    son[0]->son[2]->son[0] = vet_value
    son[0]->son[2]->son[0]->son[0] = lit
    son[0]->son[2]->son[0]->son[1] = vet_value
    */

   if(node->son[0] && node->son[0]->son[2] && node->son[0]->son[2]->son[0]) {
       
       AST* vet_value = node->son[0]->son[2]->son[0];

       if(vet_value->type == 41) {
            do {
                    AST* lit = vet_value->son[0];

                    set_values = tacJoin(set_values, tacCreate(TAC_AATTR, node->symbol, makeIndex(i), lit->symbol));

                    i++;
            } while((vet_value = (vet_value->son[1])) && vet_value->type == 41);
       }

       set_values = tacJoin(set_values, tacCreate(TAC_AATTR, node->symbol, makeIndex(i), vet_value->symbol));
   }


    // return tacJoin(code0, tacJoin(decl,code1));
    return tacJoin(code0, tacJoin(decl, tacJoin(set_values,code1)));
}
